 ...
 * Works on raw text extracted from PDFs or filenames
 *
 * Classifies: invoice | utility_bill | salary_slip | purchase_order | delivery_challan
 * Extracts: GSTIN, amounts, energy_kwh, CO2e, employees, wages, transport, HSN codes, CBAM flags
 */

// ── Document type classifier ──────────────────────────────────────────────────
export function classifyDocument(filename = '', text = '') {
  const s = (filename + ' ' + text).toLowerCase();
  if (/salary|payslip|pay.?slip|wage|epf|esi|employee/i.test(s))   return 'salary_slip';
  ...
  if (/purchase.?order|p\.?o\.?\s*no|po.?ref|vendor/i.test(s))    return 'purchase_order';
  if (/invoice|inv.?no|gstin|igst|cgst|sgst|bill.?to|tax.?invoice/i.test(s)) return 'invoice';
  return 'unknown';
}
...
export function extractEsgFromText(text = '', docType = 'unknown') {
  ...
  // ── CBAM detection (titanium, steel, aluminium, iron) ────────────────────
...
// ── Convenience: extract from filename only (for local testing) ───────────────
