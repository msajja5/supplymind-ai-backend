/**
 * store.js — In-memory store with supplier profiles
 * Swap save* methods with Supabase inserts for production.
 */
...
const db = {
  ...
  ingest_log:       [],
  batches:          {},
  supplier_profiles: {},  // keyed by supplier_id
};
...
export const store = {
  ...
  // ─ Supplier profiles (green config + state)
  saveSupplierProfile(id, profile) {
    db.supplier_profiles[id] = { ...db.supplier_profiles[id], ...profile, updated_at: new Date().toISOString() };
  },
  getSupplierProfile(id) {
    return db.supplier_profiles[id] ?? {};
  },
  getAllSupplierProfiles() {
    return db.supplier_profiles;
  },
  ...
  // ─ Batches
  saveBatch(batch) {
    if (batch.batch_id) db.batches[batch.batch_id] = batch;
    db.ingest_log.push({ event: 'batch', ...batch, logged_at: new Date().toISOString() });
  },
  getBatch(batchId)  { return db.batches[batchId] ?? null; },
  ...
  stats() {
    return {
      ...
      supplier_profiles: Object.keys(db.supplier_profiles).length,
  ...
  reset() {
    Object.keys(db).forEach(k => { db[k] = Array.isArray(db[k]) ? [] : {}; });
  },
};
