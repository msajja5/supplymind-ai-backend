...
// ────────────────────────────────────────────────────────────
test('invoice by filename',          () => eq(classifyDocument('INV-ZAP-2026.pdf',''), 'invoice'));
...
test('Webhook verify correct token → challenge', () => assert(String(g4.body).includes('HELLO_SUPPLYMIND') || g4.status === 200));
...
const waInv = await fetchLocal('/api/ingest/whatsapp', 'POST', {
  entry: [{ changes: [{ value: { messages: [{
    id: `SIM-INV-${TS}`, from: '+919988776655', type: 'document', timestamp: `${TS}`,
    document: { id: 'FAKE-INV-001', mime_type: 'application/pdf', filename: 'invoice_zenith_april_2026.pdf' }
  }] } }] }]
});
...
const waUtil = await fetchLocal('/api/ingest/whatsapp', 'POST', {
  entry: [{ changes: [{ value: { messages: [{
    id: `SIM-UTIL-${TS}`, from: '+919988776655', type: 'document', timestamp: `${TS}`,
    document: { id: 'FAKE-UTIL-001', mime_type: 'application/pdf', filename: 'electricity_bill_march_2026.pdf' }
  }] } }] }]
});
test('POST electricity bill → 200',          () => eq(waUtil.status, 200));

// Simulate salary slip
const waSal = await fetchLocal('/api/ingest/whatsapp', 'POST', {
  entry: [{ changes: [{ value: { messages: [{
    id: `SIM-SAL-${TS}`, from: '+919988776655', type: 'document', timestamp: `${TS}`,
    document: { id: 'FAKE-SAL-001', mime_type: 'application/pdf', filename: 'salary_payslip_march_2026.pdf' }
  }] } }] }]
});
...
test('3 wa_messages stored after simulation', () => assert(status2.body?.tables?.wa_messages >= 3));
...
test('POST /api/ingest → passport created',   () => assert(ing.status === 200 || ing.status === 201));
...
if (fail === 0) {
  console.log(`  \x1b[32m⭐ ALL TESTS PASSED — SupplyMind AI pipeline is healthy!\x1b[0m`);
} else {
  console.log(`  \x1b[33m⚠️  ${fail} test(s) failed (server tests need: node server.js)\x1b[0m`);
